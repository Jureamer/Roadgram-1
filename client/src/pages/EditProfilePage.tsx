...
function EditProfilePage(): any {
  ...
  const accessTokenRequest = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/users/auth?user=${auth.userInfo.id}&loginMethod=${auth.userInfo.loginMethod}`,
      {
  ...
  const submitHandler = async () => {
    ...
    if (editInform.nickname && editInform.nickname !== userInfo.nickname) {
      if (editInform.nickname.length === 1 || editInform.nickname.length > 15)
        return alert('닉네임은 2~15자 이내로 입력바랍니다.')
