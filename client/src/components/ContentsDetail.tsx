...
function ContentsDetail() {
  ...
  const createComment = async () => {
    if (isLogin && comment) {
      ...
      await axios.get(`${process.env.REACT_APP_API_URL}/comments?id=${articleInfo.id}&page=${1}`).then(res => {
      ...
      <div className="detail-body">
        <div className="detail-nav">
          <div className="writer-info">
            <img
              ...
              alt="profile_image"
              src={writerInfo.profileImage}
              onClick={() => {
                navigate(`/userinfo?id=${writerInfo.id}`)
              }}
              onKeyDown={() => {
                navigate(`/userinfo?id=${writerInfo.id}`)
              }}
            ...
            <div className="nickname_time-box">
              ...
              <span className="written_time">{`${String(articleInfo.createdAt).slice(0, 10)} ${String(
          ...
          {userInfo.id === writerInfo.id ? (
            <div className="article-buttons">
              <button className="post-delete" type="button" onClick={deleteArticle}>
                삭제
              </button>
              <button className="post-modify" type="button" onClick={moveToEdit}>
                수정
              </button>
            </div>
          ) : null}
        ...
        <div className="detail-infinite-scroll">
          ...
          <div className="post-tags">
            {articleInfo.tags
              ? articleInfo.tags.map(
                  (tag: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined) => {
                    return (
                      <div>
                        <span
                          className="each-tags"
                          key={articleInfo.tags.indexOf(tag)}
                          role="search"
                          onClick={() => navigate(`/search?tag=${tag}`)}
                          onKeyDown={() => navigate(`/search?tag=${tag}`)}
                        >
                          #{tag}
                        </span>
                      </div>
          ...
          <div className="post-comments">
            {commentInfo.length !== 0 ? (
              commentInfo.map(each => {
                return (
                  <li className="comment_list" key={each.id}>
                    ...
                    <img
                      ...
                      onClick={() => {
                        navigate(`/userinfo?id=${each.userId}`)
                      }}
                      onKeyDown={() => {
                        navigate(`/userinfo?id=${each.userId}`)
                      }}
                    ...
                    <div className="comment-info">
                      <div className="writer-nickname-time-delete-box">
                        ...
                        <span className="comment-written-time">{`${String(each.createdAt).slice(0, 10)} ${String(
                          each.createdAt,
                        ).slice(11, 16)}`}</span>
      ...
      <div className="detail-footer">
        <div className="iconBox">
          {articleInfo.likedOrNot ? (
            <FontAwesomeIcon
              className="likeUnlikeIcon"
              icon={['fas', 'heart']}
              onClick={likeUnlikeHandler}
              onKeyDown={likeUnlikeHandler}
            />
          ) : (
            <FontAwesomeIcon
              className="likeUnlikeIcon"
          ...
          <FontAwesomeIcon className="commentIcon" icon={['far', 'comment-dots']} />
        ...
        <div className="comment-writing">
          ...
          {!isUpdatable ? (
            <div className="comment-writing-box">
              ...
              />
              <button type="submit" className="comment-submit" onClick={createComment}>
                작성
              </button>
            </div>
          ) : (
            <div className="comment-writing-box">
              ...
              <button type="submit" className="comment-update" onClick={() => saveComment(targetCommentId)}>
                수정
              </button>
            </div>
          )}
